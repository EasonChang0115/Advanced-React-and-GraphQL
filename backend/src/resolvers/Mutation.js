...
const { randomBytes } = require('crypto'); // 亂數產生字串 
...
const Mutations = {
  ...
  async signin(parent, { email, password }, ctx ,info) {
    ...
    // 2. check if their password is connect
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    const [user] = await ctx.db.query.users({
      where: {
        ...
        resetTokenExpiry_gte: Date.now() - 3600000, // resetTokenExpiry_gte 大於resetTokenExpiry的值
      },
    });
    if (!user) {
